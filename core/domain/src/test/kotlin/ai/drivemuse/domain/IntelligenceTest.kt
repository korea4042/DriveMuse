...
class IntelligenceTest {
    ...
    @Test fun unknownGenreFailsHardExclusion() { assertFalse(Constraints(excludedGenres=setOf("POP")).allows(Track("abcdefghijk","A","B"))) }
    @Test fun unsupportedFeatureRejected() { assertFalse(Constraints(required=mapOf("genre" to setOf("POP"))).allows(Track("abcdefghijk","A","B",features=listOf(VerifiedFeature("genre","POP","",1.0))))) }

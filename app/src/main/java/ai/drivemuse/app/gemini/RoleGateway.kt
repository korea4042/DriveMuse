...
enum class Role(val wire: String, val version: String) { SURVEY("taste-intake","2.1"), CONTEXT("context-interpreter","2.1"), REVIEW("listening-reviewer","2.1"), SELECTOR("selector","2.4"), METADATA("metadata-interpreter","2.3"), DISCOVERY("discovery-planner","2.2") }
object JsonGate {
...
class RoleGateway(
    private val context: Context, private val dao: IntelligenceDao,
    private val configProvider: () -> Map<String, String> = { emptyMap() },
    private val directProvider: () -> Map<String, String> = { emptyMap() }
) {
    ...
    private val direct get() = directProvider()
    ...
    /** §22: the personal key path is an explicit choice and takes precedence when it is configured. */
    private val directClient: GeminiDirectClient? get() {
        val d = direct
        val key = d["apiKey"]?.trim().orEmpty(); val model = d["modelId"]?.trim().orEmpty()
        return if (key.isNotBlank() && model.isNotBlank()) GeminiDirectClient(key, model) else null
    }
    val configured get() = directClient != null || (firebaseApp != null && modelId.isNotBlank())
    suspend fun call(role: Role, input: JSONObject, payloadSchema: Schema, validate: (JSONObject)->Unit): JSONObject = withTimeout(20000) {
        ...
        // Both paths return JSON that the same gate below verifies; the direct path has no response
        // schema, so the envelope and payload checks are the only thing standing between model output
        // and the queue — which was already true for the managed path.
        val text=directClient?.generateJson(system, request.toString())
            ?: run {
                ...
                model.generateContent(request.toString()).text ?: error("Empty response")
            }
        require(text.length<=16000)
